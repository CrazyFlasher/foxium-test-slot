...
export abstract class AbstractScript
{
    ...
    protected getFilePathList(path: string, ext: string, subDirs = true): { name: string; path: string } []
    {
        ...
        if (this.fs.existsSync(path) && this.fs.lstatSync(path).isDirectory())
        {
            ...
            while ((dirent = dir.readSync()) !== null)
            {
                ...
                if (this.fs.lstatSync(p).isDirectory())
                {
                    if (subDirs)
                    {
                        filePaths = filePaths.concat(this.getFilePathList(p, ext, subDirs));
                    }
                }
                else
                {
                    if (fileName.substring(fileName.length - ext.length - 1) == "." + ext)
                    {
